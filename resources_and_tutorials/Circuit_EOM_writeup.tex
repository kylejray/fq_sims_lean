...
\[ \partial_{\df\varphi_{dc}} \frac{a}{2} \sqrt\frac{\mu'}{\mu} F(\dl\varphi, \dl\varphi_{dc})  = - \partial_{\df\eta} \frac{1}{2}  F(\frac{\dl\varphi_{dc}}{2}, 2\dl\varphi)\]

\[ \iff \frac{a}{2} \sqrt\frac{\mu'}{\mu} \fpderiv{\dl\varphi_{dc}} {\df\varphi_{dc}} g(\dl\varphi,\dl\varphi_{dc})  = -    \fpderiv{\dl\eta} {\df\eta} \fpderiv{\dl\varphi} {\dl\eta} \partial_{2\dl\varphi} F(\frac{\dl\varphi_{dc}}{2}, 2\dl\varphi) \]

\[ \iff \frac{a}{2} \sqrt\frac{\mu'}{\mu}  g(\dl\varphi,\dl\varphi_{dc})  = - \left(-\frac{1}{a} \sqrt\frac{\mu}{\mu'}\right)) g(\frac{\dl\varphi_{dc}}{2}, 2\dl\varphi) \]

The relation we derived for $g$ shows that the two functions are identical, so 
\[ \iff \frac{a}{2} \sqrt\frac{\mu'}{\mu}   = \frac{1}{a} \sqrt\frac{\mu}{\mu'} \]

The relation above is equivalent to the condition $\mu'=\mu$, so we choose a symmetric device. Note that this could be remedied by choosing a scaled version of the $\dl\varphi_{dc}$ coordinates that also depend on the mutual inductances-- but let's stick with the symmetric assumption for now. We can use the same method to argue the rest also work out-- though I think it might be a little over the top to write each case down here.

Note that choosing a symmetric device reduces our coordinates to:

\begin{align}
 &\dl\varphi =\frac{1}{\sqrt{2}} (-\dl\eta+\dl\zeta) &\dl\varphi' =\frac{1}{\sqrt{2}} (\dl\eta+\dl\zeta)
 \end{align}
 or, if we look at it the other way
  \begin{align}
 &\df\eta = \frac{1}{\sqrt2} \left( -  \df\varphi + \df\varphi' \right)
 &\df\zeta = \frac{1}{\sqrt2} \left( \df\varphi + \df\varphi' \right)
 \end{align}
...
\begin{multline}
U_C = -\frac{\Phi_0}{2\pi}\cdot  I_+ \left[ \cos\dl\varphi\cos\frac{\dl\varphi_{dc}}{2} + \cos\dl\varphi'\cos\frac{\dl\varphi'_{dc}}{2} \right] + \\
\frac{\Phi_0}{2\pi}\cdot  I_- \left[ \sin\dl\varphi\sin\frac{\dl\varphi_{dc}}{2} + \sin\dl\varphi'\sin\frac{\dl\varphi'_{dc}}{2} \right]
\end{multline}

It has been constructed to satisfy the $\df\varphi_{dc}, \df\varphi'_{dc}$ parts of the set of equations $m_i \ddot{x}_i = -\partial_{x_i} U(\vec{x})$ where $m_{dc} = m'_{dc} = \frac{C}{4} $. Now, we can check if this potential works by checking If $- \sqrt 2\partial_{\df\eta} U_C = F(\dl\varphi, \dl\varphi_{dc}) - F(\dl\varphi', \dl\varphi'_{dc}) $.

\begin{align*}
 -\sqrt2 \partial_{\df\eta} U_C &= - \sqrt2 \fpderiv{\dl\eta} {\df\eta} \fpderiv{\dl\varphi} {\dl\eta} \fpderiv{U_c} {\dl\varphi} - \sqrt 2\fpderiv{\dl\eta} {\df\eta} \fpderiv{\dl\varphi'} {\dl\eta} \fpderiv{U_c} {\dl\varphi'}\\
...
The same process verifies that this potential also satisfies $-\partial_{\df\zeta} U_C = \frac{1}{\sqrt2} F(\varphi, \varphi_{dc}) +\frac{1}{\sqrt2} F(\varphi', \varphi'_{dc}) $. Taking into consideration the quadratic terms as well, we have:

\begin{align*}
U &= \frac{1}{2L(1-\mu)} (\df\eta-\df\eta_x)^2 + \frac{1}{2L(1+\mu)} (\df\zeta-\df\zeta_x)^2 \\
 &+ \frac{\gamma}{2L} \left( (\df\varphi_{dc}-\df\varphi_{xdc})^2 + (\df\varphi'_{dc}-\df\varphi'_{xdc})^2\right) + U_c
\end{align*}
