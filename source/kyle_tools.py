...
def animate_sim(all_state, total_time, frame_skip=30, which_axes=None, axes_names=None, color_by_state=None, key_state=None, color_key=None, legend=True):
    ...
    if len(x_array) == 2:
        ...
        if color_by_state is None:
            ...
            if color_key is not None:
                ...
                scat = [plt.scatter(x[state_lookup[key], 0], y[state_lookup[key], 0], c=color_lookup[key], alpha=min(1, 300/N)) for key in state_lookup]
            else:
    ...
    if len(x_array) == 3:
        ...
        if color_by_state is None:
            ...
            if color_key is not None:
                ...
                scat = [ax.scatter(x[state_lookup[key], 0], y[state_lookup[key], 0], z[state_lookup[key], 0], alpha=min(1, 300/N)) for key in state_lookup]
            if legend:
                fig.legend(state_lookup)
...
def animate_hist_1D(all_state, total_time, which_axes=None, frame_skip=20, nbins=64, lims=None):
    ...
    for j in range(len(which_axes)):
        h1 = ax.hist(coords[j][:, 0], bins=nbins, range=lims, density=True)
        y_max = np.max(h1[0])
    ...
    def animate(i):
        ...
        ax.set_ylim([0, 1.2 * y_max])
        for j in range(len(which_axes)):
            ax.hist(coords[j][:, i], bins=nbins, range=lims, density=True)
